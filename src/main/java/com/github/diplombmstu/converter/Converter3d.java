...
//        double Ppx = wsm / wpx / Psm;
        ...
        if (lx < inputWidth)
        {
            ...
            leftImage.setRGB(i, j, rx >= 0 ? inputImage.getRGB(rx, j) : inputColor);
        }

        if (rx >= 0)
        {
            rightImage.setRGB(rx, j, inputColor);
            rightImage.setRGB(i, j, lx < inputWidth ? inputImage.getRGB(lx, j) : inputColor);
        }
    }

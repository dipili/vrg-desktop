...
final class CameraStreamer extends Object
{
    ...
    private void tryStartStreaming()
    {
        try
        {
            while (true)
            {
                ...
                {
                    Log.d(TAG,
                          "Open camera failed, retying in " + OPEN_CAMERA_POLL_INTERVAL_MS + "ms",
                          openCameraFailed);
    ...
    private void sendPreviewFrame(final byte[] data, final Camera camera, final long timestamp)
    {
        ...
        // Create JPEG
        final YuvImage image = new YuvImage(data, mPreviewFormat, mPreviewWidth, mPreviewHeight, null /* strides */);
